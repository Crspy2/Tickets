...
class TicketDB:
    ...
    def create_ticket(self, channel: GuildChannel, user: User, panel_id: int = 0):
        ...
        ticket_info = {
            ...
            "ticket_id": self.settings.find_one({"guild_id": self.guild.id}),
    ...
    def get_ticket_info(self, ticket_channel: GuildChannel) -> Mapping[str, any] | None:
        guild = self.settings.find_one({"channel_id": ticket_channel.id})
        return guild
    ...
    def update_ticket_info(self, ticket_channel: GuildChannel, **kwargs) -> int:
        ...
        for key, value in kwargs.items():
            ...
            result = self.settings.update_one({"channel_id": ticket_channel.id}, update_query)
        return result.modified_count

    def delete_ticket(self, ticket_channel: GuildChannel) -> int:
        result = self.settings.delete_one({"channel_id": ticket_channel.id})
